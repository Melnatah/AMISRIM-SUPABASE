...
const Sidebar: React.FC<SidebarProps> = ({ sites, user }) => {
            ...
            <div className="flex flex-col gap-1 ml-9 border-l border-dark-border pl-3 mt-1 animate-in slide-in-from-top-2 duration-300">
              {item.subItems.map((sub: any) => (
                <Link
                  key={sub.path}
                  to={sub.path}
                  className={`py-2 text-xs font-medium transition-colors truncate pr-2 ${
                    active === sub.path ? 'text-primary' : 'text-slate-500 hover:text-slate-300'
                  }`}
                >
                  {sub.label}
                </Link>
    ...
    if (item.type === 'external') {
      return (
        <button
          key={item.label}
          onClick={item.action}
          className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-slate-400 hover:text-white hover:bg-white/5 transition-colors"
        >
          <span className="material-symbols-outlined">{item.icon}</span>
          <span className="text-sm font-medium">{item.label}</span>
  ...
  return (
    <aside className="hidden lg:flex flex-col w-64 bg-[#0e1117] border-r border-[#2d3342] z-20 h-full font-jakarta">
      <div className="flex-none h-16 flex items-center px-6 border-b border-[#2d3342]">
        ...
        {user.role === 'admin' && (
          <div className="pt-6 mt-6 border-t border-dark-border">
            ...
            <Link
              to="/admin"
              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors ${
                active === '/admin' ? 'bg-amber-500/10 text-amber-500' : 'text-slate-400 hover:text-amber-500 hover:bg-amber-500/5'
              }`}
            >
